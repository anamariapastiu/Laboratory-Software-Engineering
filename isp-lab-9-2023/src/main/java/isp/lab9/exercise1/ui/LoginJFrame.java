...
public class LoginJFrame extends JFrame {
    ...
    private void verifyCredentials(JTextField userInputField, JPasswordField pwdInputField) {
        ...
        String usr=userInputField.getText();
        String pwd=new String(pwdInputField.getPassword());

        if(accounts.get(usr)!=null){
            System.out.println(accounts.get(usr));
            if(accounts.get(usr).equals(pwd)) {
                LoginJFrame.this.setVisible(false);
                new StockMarketJFrame();
                return;
            }
